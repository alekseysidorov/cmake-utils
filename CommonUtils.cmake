...
macro(LIST_FILTER list regex output)
    foreach(item list)
        string(REGEX MATCH regex item ${item})
        if(item)
...
function(__CHECK_SOURCE_FILES name)
    ...
    moc_wrap_cpp(MOC_SRCS ${HDR})
    qt4_add_resources(QRC_SOURCES ${QRC})
    list(APPEND extra_sources ${CXX} ${CC} ${MM} ${HDR} ${UIS_H} ${MOC_SRCS} ${QRC_SOURCES})
    set(${name} ${extra_sources} PARENT_SCOPE)
endfunction()
...
macro(ADD_SIMPLE_LIBRARY target)
    ...
    __check_source_files(EXTRA_SOURCES ${LIBRARY_SOURCE_FILES})

    # This project will generate library
    add_library(${target} ${type} ${LIBRARY_SOURCES} ${EXTRA_SOURCES})
